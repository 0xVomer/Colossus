...
pub mod zkp_schnorr_fiat_shamir {
    ...
    pub fn non_interact_prove(
        ...
        let witness_list =
            w_list.iter().map(G2Projective::mul_by_generator).collect::<Vec<G2Projective>>();

        // sum all w_list items into announcement
        let announcement = witness_list.iter().fold(G2Projective::IDENTITY, |acc, x| acc + x);
...
mod tests {
    ...
    fn test_interact_prove() {
        ...
        assert!(ZKPSchnorr::verify(&challenge, &announce.0, &stm.into(), &response));
    }
