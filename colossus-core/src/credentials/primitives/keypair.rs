...
use crate::{CredentialCompressed, config, error};
use crate::{
    zkp::PedersenOpen,
    zkp::Schnorr,
...
pub use delano_keys::vk::{VK, VKCompressed};
...
impl std::fmt::Display for IssuerError {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            ...
            IssuerError::InvalidNymProof => {
                write!(f, "InvalidNymProof. The proof of the pseudonym is invalid.")
            },
...
pub trait CBORCodec {
        ...
        Self: Sized + Serialize,
    {
        ...
        match ciborium::into_writer(&self, &mut bytes) {
            Ok(_) => Ok(bytes),
            Err(e) => {
                Err(error::Error::CBORError(format!("CBORCodec Error serializing to bytes: {}", e)))
            },
        }
    }
...
impl<Stage> Nym<Stage> {
    ...
    fn new_from_secret(secret_bytes: Secret<Scalar>) -> Nym<Stage> {
        ...
        Nym::<Stage> {
            ...
            secret: secret_bytes,
            public: NymPublic { damgard: DamgardTransform::new(), key },
        }
    }
    ...
    pub fn extend(
        ...
        let cred =
            spseq_uc::change_rel(&cred.issuer_public.parameters, addl_attrs, cred.clone(), &mu)
                .map_err(|e| {
    ...
    pub fn accept(
        ...
        {
            return Err(error::Error::AcceptOfferFailed(String::from("Invalid Signature")));
        }
...
mod tests {
    ...
    fn setup_tests() -> TestMessages {
        ...
        TestMessages { message1_str, message2_str, message3_str }
    }
    ...
    fn test_sign() -> Result<(), IssuerError> {
        ...
        let cred = issuer.sign(
            &user.public.key,
            &[Entry(messages_vectors.message1_str), Entry(messages_vectors.message2_str)],
            None,
        )?;
    ...
    fn test_change_rel_from_sign() -> Result<(), IssuerError> {
        ...
        let k_prime = 3;
        let messages_vector =
            &vec![Entry(messages_vectors.message1_str), Entry(messages_vectors.message2_str)];
        ...
        assert_eq!(
            signature_original.update_key.as_ref().expect("There to be an update key").len(),
            k_prime
        );
        ...
        for k in 0..messages_vector.len() {
            assert_eq!(
                signature_original.update_key.as_ref().expect("There to be an update key")[k],
                Vec::new()
            );
        }
        ...
        let signature_changed =
            spseq_uc::change_rel(&issuer.public.parameters, &message_l, signature_original, &mu)?;
    ...
    fn test_convert() -> Result<(), error::Error> {
        ...
        let signature_original =
            issuer.sign(&nym.public.key, messages_vector, k_prime).expect("valid tests");
    ...
    fn test_issue_root_cred() -> Result<(), IssuerError> {
        ...
        let cred =
            issuer.issue_cred(messages_vector, k_prime, &nym.nym_proof(&NONCE), Some(&NONCE))?;

        // check the correctness of root credential
        // assert (spseq_uc.verify(pp_sign, vk_ca, nym_u, commitment_vector, sigma))
        assert!(verify(&issuer.public.vk, &nym.public.key, &cred.commitment_vector, &cred.sigma));
        Ok(())
    }
    ...
    fn test_delegate_only() -> Result<()> {
        ...
        let k_prime = Some(5);
        let messages_vector =
            &mut vec![Entry(messages_vectors.message1_str), Entry(messages_vectors.message2_str)];
        ...
        // verify_proof
        assert!(verify_proof(&issuer.public, &proof, messages_vector, Some(&NONCE)));
    ...
    fn test_prove_subset_creds() -> Result<(), IssuerError> {
        ...
        let all_attributes =
            vec![Entry::new(&message1_str), Entry::new(&message2_str), Entry::new(&message3_str)];
        ...
        let k_prime = Some(4);
        let cred =
            issuer.issue_cred(&all_attributes, k_prime, &nym_p.nym_proof(&NONCE), Some(&NONCE))?;
        ...
        // verify_proof
        assert!(verify_proof(&issuer.public, &proof, &selected_attrs, Some(&NONCE)));
    ...
    fn test_delegate_subset() -> Result<()> {
        ...
        // age is from Entry[0]
        let selected_attrs =
            vec![Entry(vec![age]), Entry(vec![]), Entry(vec![insurance]), Entry(vec![legal_age])];
    ...
    fn test_second_offer() -> Result<()> {
        ...
        assert_eq!(bobby_offer.commitment_vector.len(), bobby_cred.commitment_vector.len() + 1);
    ...
    fn test_offer_compressed_roundtrip() {
        ...
        // extend the credential
        let extended_cred =
            nym.extend(&cred, &Entry(messages_vectors.message2_str.clone())).unwrap();
