...
use ed25519_dalek::{
    self, PUBLIC_KEY_LENGTH, SECRET_KEY_LENGTH, VerifyingKey as ed25519_PublicKey,
};
...
fn test_output_from_proof() {
    for tv in TESTVECTORS.iter() {
        assert_eq!(
            tv.beta,
            to_string!(Output::from(&from_string!(VRFPrivateKey, tv.SK).prove(tv.alpha)))
        );

        assert_eq!(tv.beta, to_string!(&from_string!(VRFPrivateKey, tv.SK).evaluate(tv.alpha)));
    }
}
