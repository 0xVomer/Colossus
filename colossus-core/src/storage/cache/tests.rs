...
use crate::{
    akd::{AkdLabel, AkdValue, NodeLabel},
    storage::types::{DbRecord, ValueState, ValueStateKey},
};

#[tokio::test]
async fn test_cache_put_and_expires() {
    let cache =
        TimedCache::new(Some(Duration::from_millis(10)), None, Some(Duration::from_millis(50)));

    let value_state = DbRecord::ValueState(ValueState {
        epoch: 1,
        version: 1,
        label: NodeLabel { label_len: 1, label_val: [0u8; 32] },
...
async fn test_cache_overwrite() {
    ...
    let value_state_2 = ValueState {
        epoch: 1,
        version: 2,
        label: NodeLabel { label_len: 2, label_val: [0u8; 32] },
        value: AkdValue::from("some value"),
        username: AkdLabel::from("user"),
    };
