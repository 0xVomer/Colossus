...
async fn test_get_and_set_item<Ns: Database>(storage: &Ns) {
    ...
    let get_result = storage.get::<Azks>(&crate::akd::DEFAULT_AZKS_KEY).await;
    ...
    let set_result =
        storage.set(DbRecord::TreeNode(PvTreeNode::from_tree_node(node.clone()))).await;
    assert_eq!(Ok(()), set_result);

    let set_result =
        storage.set(DbRecord::TreeNode(PvTreeNode::from_tree_node(node2.clone()))).await;
    assert_eq!(Ok(()), set_result);
...
async fn test_batch_get_items<Ns: Database>(storage: &Ns) {
    ...
    let got = storage.get::<ValueState>(&ValueStateKey(rand_users[0].clone(), 10)).await;
    if got.is_err() {
        panic!("Failed to retrieve a user after batch insert");
    }

    let keys: Vec<ValueStateKey> =
        rand_users.iter().map(|user| ValueStateKey(user.clone(), 1)).collect();
...
async fn test_transactions<S: Database>(storage: &StorageManager<S>) {
    ...
    data.push(DbRecord::Azks(Azks { latest_epoch: 1, num_nodes: 34 }));
...
async fn test_user_data<S: Database>(storage: &S) {
    ...
    let result = storage.set(DbRecord::ValueState(sample_state.clone())).await;
    assert_eq!(Ok(()), result);
    ...
    */
    ...
    let specific_result = storage
        .get_user_state(&sample_state.username, ValueStateRetrievalFlag::SpecificVersion(2))
        .await;
    assert_eq!(
        Ok(ValueState {
            epoch: 123,
    ...
    let missing_result = storage
        .get_user_state(&sample_state.username, ValueStateRetrievalFlag::SpecificVersion(100))
        .await;
    assert!(matches!(missing_result, Err(StorageError::NotFound(_)),));

    let specific_result = storage
        .get_user_state(&sample_state.username, ValueStateRetrievalFlag::SpecificEpoch(123))
        .await;
    assert_eq!(
        Ok(ValueState {
            epoch: 123,
    ...
    let records = vector_of_states.into_iter().map(DbRecord::ValueState).collect::<Vec<_>>();
    let result = storage.batch_set(records, crate::storage::traits::DbSetState::General).await;
    assert_eq!(Ok(()), result);
...
) -> Result<(), crate::akd::errors::AkdError> {
    let rand_user = rng()
        .sample_iter(&Alphanumeric)
        .take(30)
        .map(char::from)
