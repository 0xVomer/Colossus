    ...
    K: Hash + PartialEq + Eq + Clone + Debug,
{
    ...
    pub fn insert(&mut self, key: K, value: V) -> Option<V> {
        match self.indices.entry(key.clone()) {
            Entry::Occupied(e) => Some(std::mem::replace(&mut self.entries[*e.get()].1, value)),
            Entry::Vacant(e) => {
    ...
    pub fn update_key(&mut self, old_key: &K, mut new_key: K) -> Result<(), Error> {
        let index_entry = *self.indices.get(old_key).ok_or(Error::missing_entry(old_key))?;
...
mod tests {
    ...
    fn test_dict() -> Result<(), Error> {
        ...
        assert_eq!(d.get_key_value("ID2").unwrap(), (&"ID2".to_string(), &"Bar".to_string()));
