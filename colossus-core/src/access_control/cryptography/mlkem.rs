// MlKem768 is generated by macro but only MlKem512 is currently used
#![allow(dead_code)]

use super::{SHARED_SECRET_LENGTH, traits::Kem};
...
macro_rules! make_mlkem {
    ($base: ident, $ek: ident, $ek_len: literal, $dk: ident, $dk_len: literal, $enc: ident, $enc_len:literal) => {
        ...
        impl Serializable for $enc {
            ...
            fn read(de: &mut Deserializer) -> Result<Self, Self::Error> {
                Ok(Self(Box::new(Array::<u8, <ml_kem::$base as KemCore>::CiphertextSize>::from(
                    de.read_array::<$enc_len>()?,
                ))))
            }
        }
        ...
        impl Kem for $base {
            ...
            fn enc(
                ...
                let (enc, mut ss) =
                    ek.0.encapsulate(rng).map_err(|e| Error::Kem(format!("{:?}", e)))?;
            ...
            fn dec(
                ...
                let mut ss =
                    dk.0.decapsulate(&enc.0).map_err(|e| Self::Error::Kem(format!("{e:?}")))?;
                let ss = Secret::from_unprotected_bytes(ss.as_mut());
                Ok(ss)
            }
        }
